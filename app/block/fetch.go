...
func FetchBlockByHash(client *ethclient.Client, hash common.Hash, number string, _db *gorm.DB, kafkaInfo *d.KafkaInfo, queue *q.BlockProcessorQueue, _status *d.StatusHolder) bool {
	...
	return ProcessBlockContent(client, block, _db, kafkaInfo, true, queue, _status, startingAt)

}

// FetchBlockByNumber - Fetching block content using block number
func FetchBlockByNumber(client *ethclient.Client, number uint64, _db *gorm.DB, kafkaInfo *d.KafkaInfo, publishable bool, queue *q.BlockProcessorQueue, _status *d.StatusHolder) bool {
	...
	return ProcessBlockContent(client, block, _db, kafkaInfo, publishable, queue, _status, startingAt)
...
func FetchTransactionByHash(client *ethclient.Client, block *types.Block, tx *types.Transaction, returnValChan chan *db.PackedTransaction) {
